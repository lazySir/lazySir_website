...
exports.addDictionary = async (req, res) => {
  try {
    ...
    const existingDictionary = await prisma.sysDictionary.findFirst({
      where: {
        key,
        level,
...
exports.updateDictionary = async (req, res) => {
  try {
    const { dictionaryId, key, value, level, description, state, parentId } =
      req.body
...
exports.deleteDictionary = async (req, res) => {
  try {
    ...
    if (cannotDelete.length > 0) {
      message +=
        '以下字典项因具有子级字典项未能删除：' +
        cannotDelete.map((item) => ` ${item.value}`).join('；')
    }
