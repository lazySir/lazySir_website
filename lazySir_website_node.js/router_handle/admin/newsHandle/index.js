...
exports.getNews = async (req, res) => {
  try {
    ...
    if (hotSearchWordIds)
      queryConditions.hotSearchWordIds = { contains: hotSearchWordIds }
    ...
    const result = await Promise.all(
      newsList.map(async (news) => {
        ...
        if (news.companyId) {
          ...
          updatedNews.companyValue = systemDictionary
...
exports.updateNews = async (req, res) => {
  try {
    const { newsId, title, content, state, hotSearchWordIds, hits, companyId } =
      req.body
...
exports.uploadNewsImg = async (req, res) => {
  try {
    ...
    // 构建图片访问 URL
    const imgUrl =
      process.env.SERVER_URL + uploadUrls.newsImage.saveUrl + `${file.filename}`
    // 返回成功响应
    return res.send({
      errno: 0,
      data: { url: imgUrl },
