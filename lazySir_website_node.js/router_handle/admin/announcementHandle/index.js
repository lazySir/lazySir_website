...
exports.updateAnnouncement = async (req, res) => {
    try {
        const { announcementId, title, content, state, companyId, file, hits } = req.body;
        // 查找正在更新的公告
        const existingAnnouncement = await prisma.announcement.findUnique({
...
exports.deleteAnnouncements = async (req, res) => {
    try {
        ...
        const existingAnnouncementIds = existingAnnouncements.map((item) => item.announcementId);
        const notFoundIds = announcementIds.filter((id) => !existingAnnouncementIds.includes(id));
...
exports.uploadAnnouncementImg = async (req, res) => {
    try {
        ...
        // 构建图片访问 URL
        const imgUrl = process.env.SERVER_URL + uploadUrls.announcementImage.imgSaveUrl + `${file.filename}`;
        // 返回成功响应
        return res.send({
            errno: 0,
            data: { url: imgUrl }
...
exports.uploadAnnouncementFile = async (req, res) => {
    try {
        ...
        const file = req.file;
        // 转换文件名为 UTF-8
        const UTFName = iconv.decode(Buffer.from(file.originalname, 'binary'), 'utf8');
        if (!file) {
