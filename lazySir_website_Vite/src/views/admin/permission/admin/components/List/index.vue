<template>
    ...
    >
      <template #default="scope">
        <el-tag
          ...
          :type="item.state ? 'primary' : 'danger'"
          :key="item.roleId"
        >
          {{ item.roleName }}
        </el-tag>
    ...
    >
      <template #default="scope">
        ...
        <AuthBtn
          ...
          perm="UPDATE"
          size="small"
          @click="resetPwd(scope.row.accountId)"
          icon="refreshLeft"
        />
        <el-popconfirm
          cancel-button-text="取消"
          confirm-button-text="确认"
          @confirm="deleteAdmin(scope.row.accountId)"
          :title="`确定删除:${scope.row.username}?`"
        >
          <template #reference>
            <AuthBtn
              type="primary"
  ...
  <div class="demo-pagination-block">
    <BasePagination
      ...
      v-model:current-page="adminUserStore.PageAndSize.page"
      v-model:page-size="adminUserStore.PageAndSize.limit"
      :total="adminUserStore.PageAndSize.total"
